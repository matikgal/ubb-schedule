...
import { Shield, LogOut, ChevronRight, Bell, Check, Palette, ChevronDown, ChevronUp, ChevronLeft, Moon, Sun, Trash2, RefreshCw, Info, Edit3, X } from 'lucide-react';
  ...
  return (
      <div className="space-y-6 animate-fade-in pt-6">
          ...
          <div className="bg-surface rounded-2xl p-6 border border-border space-y-4 text-muted text-sm leading-relaxed shadow-sm">
              <p>
                  Twoja prywatność jest dla nas priorytetem. Aplikacja <strong className="text-main">UniSchedule</strong> przechowuje Twoje preferencje (wybrana grupa, motyw) wyłącznie lokalnie na Twoim urządzeniu (Local Storage).
              </p>
              <p>
                  Żadne dane osobowe nie są przesyłane na zewnętrzne serwery. Dane o planie zajęć są pobierane z publicznie dostępnych źródeł uczelni lub wprowadzane ręcznie.
              </p>
              <p>
                 Aplikacja nie śledzi Twojej lokalizacji ani nie wykorzystuje plików cookies w celach reklamowych.
              </p>
          </div>
      </div>
  );
...
const SettingsPage = () => {
    ...
    const avatarOptions = [
      'Alexander', 'Aneka', 'Felix', 'Jake', 'Jocelyn', 
      'Micah', 'Minia', 'Robert', 'Sorell', 'Zoe'
    ];
                    ...
                    {isAvatarExpanded && (
                      <div className="mt-6 mb-2 grid grid-cols-5 gap-3 animate-slide-down bg-background/50 p-4 rounded-2xl">
                         {avatarOptions.map(seed => (
                           <button 
                              key={seed}
                              onClick={() => { setAvatarSeed(seed); setIsAvatarExpanded(false); }}
                           ...
                           >
                              <img src={`https://api.dicebear.com/9.x/notionists/svg?seed=${seed}&backgroundColor=b6e3f4`} alt={seed} />
                           </button>
                         ))}
                      </div>
                    )}
                       ...
                       {isEditingName ? (
                         <div className="flex items-center gap-2 bg-background rounded-lg px-2 py-1 border border-primary/50">
                            <input 
                              ...
                              autoFocus
                            />
                            <button onClick={handleNameSave}><Check size={18} className="text-green-500" /></button>
                         </div>
                       ) : (
                         <h2 
                ...
                <div className="bg-surface rounded-2xl p-4 border border-border shadow-sm">
                    {groupData ? (
                        ...
                        <div className="flex justify-between items-center text-muted text-sm">
                            ...
                            <button onClick={() => navigate('/search')} className="text-primary text-xs font-bold">Wybierz</button>
                        </div>
                    )}
                </div>
            </section>
                 ...
                 <div className="bg-surface rounded-2xl overflow-hidden border border-border shadow-sm">
                     ...
                     <div className="transition-all">
                        ...
                        >
                            ...
                            <div className="flex items-center gap-3">
                                <div className="w-3 h-3 rounded-full shadow-sm border border-white/10" style={{ backgroundColor: currentTheme.hex }}></div>
                                {isThemeExpanded ? <ChevronUp size={16} className="text-muted" /> : <ChevronDown size={16} className="text-muted" />}
                            </div>
                        </div>
                        ...
                        {isThemeExpanded && (
                            <div className="p-4 bg-background/30 flex gap-4 overflow-x-auto no-scrollbar animate-slide-down border-t border-border">
                                {THEME_COLORS.map((theme) => (
                                    ...
                                    >
                                        <div 
                                            className={`w-9 h-9 rounded-full flex items-center justify-center transition-all ${currentTheme.id === theme.id ? 'ring-2 ring-primary scale-110' : 'opacity-70 group-hover:opacity-100'}`}
                                            style={{ backgroundColor: theme.hex }}
                                        >
                                            {currentTheme.id === theme.id && <Check size={14} className={theme.id === 'white' && !isDarkMode ? 'text-white' : 'text-black'} />}
                                        </div>
                                        <span className={`text-[9px] font-medium ${currentTheme.id === theme.id ? 'text-primary' : 'text-muted'}`}>
                                            {theme.name}
                                        </span>
                                    </button>
                                ))}
                 ...
                 <div className="bg-surface rounded-2xl overflow-hidden border border-border shadow-sm">
                     ...
                     >
                         <div className="flex items-center gap-3">
                             ...
                             <span className="text-main text-sm font-medium group-hover:text-red-400 transition-colors">Wyczyść historię</span>
                         </div>
                     </div>

                     <div onClick={() => navigate('/privacy')} className="p-4 flex items-center justify-between hover:bg-hover transition-colors cursor-pointer border-t border-border">
                         <div className="flex items-center gap-3">
            ...
            <div className="text-center space-y-4 pt-4 border-t border-border">
                ...
                <div className="space-y-1">
                    <p className="text-[10px] text-muted">
                        Dane planu są własnością Uniwersytetu Bielsko-Bialskiego.
                    </p>
                    <div className="text-[10px] text-muted font-medium pt-1 space-y-0.5">
                        <p>Aplikacja: <span className="text-main font-bold">Mateusz Gałuszka</span></p>
                        <p>Pobieranie danych (Scraper): <span className="text-main font-bold">Jakub Gałosz</span></p>
                    </div>
                    <p className="text-[9px] text-muted/50 pt-2">
                        Assets: DiceBear (Avatars), Unsplash, Lucide Icons
                    </p>
                </div>
            </div>
        </div>
