...
export async function fetchScheduleForWeek(groupId: number, weekId?: number | string): Promise<ClassEvent[]> {
	try {
...
async function updateScheduleInBackground(groupId: number, weekId?: number | string): Promise<void> {
	try {
		if (!supabase) return
 ...
 */
export async function getAvailableWeeks(groupId: number): Promise<Array<{ id: string; label: string; start: Date; end: Date }>> {
	if (!isSupabaseAvailable || !supabase) {
		return [];
	}
